...
class CSharpAuditEngine:
    ...
    def run_all_audits(self) -> AuditResult:
        ...
        for audit_method in audit_methods:
            ...
            try:
                ...
                if not all(isinstance(v, ArchitecturalViolation) for v in result):
                    error_msg = f"Audit method {method_name} returned non-violation items"
                    logger.error(error_msg)
                    raise TypeError(error_msg)
                ...
                raise RuntimeError(f"Audit failed: {method_name} raised unexpected error") from unexpected_error
        ...
        if types_by_count > 0:
            avg_lcom = sum(
                t.lcom_score for t in self.analyzer.types.values()
            ) / types_by_count

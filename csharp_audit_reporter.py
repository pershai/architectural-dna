...
Uses atomic_file_write from export_utils for consistency with universal exporter.
"""
...
from export_utils import atomic_file_write
...
class CSharpAuditReporter:
    ...
    def generate_json_report(
        ...
        if types:
            ...
            for type_name, type_info in types.items():
                try:
                    if (
                        not hasattr(type_info, "design_patterns")
                        or not type_info.design_patterns
                    ):
                        continue
                    ...
                    for attr in required_attrs:
                        if not hasattr(type_info, attr):
                            logger.warning(
                                f"Type '{type_name}' missing required attribute '{attr}'"
                            )
                            continue
                    ...
                    logger.warning(
                        f"Invalid type_info structure for '{type_name}': {e}"
                    )
                    continue
        ...
        # Use atomic write for data integrity (temp → rename)
        def _write_json(temp_file: Path) -> None:
            with open(temp_file, "w", encoding="utf-8") as f:
                json.dump(report, f, indent=2)

        write_result = atomic_file_write(output_path, _write_json)

        if write_result.success:
            logger.info(
                f"JSON report written to: {output_path} "
                f"({write_result.file_size} bytes, {write_result.duration_seconds:.2f}s)"
            )
        else:
            logger.error(f"Failed to write JSON report: {write_result.error}")
            raise ValueError(f"Cannot write report: {write_result.error}")
    ...
    def generate_markdown_report(
        ...
        # Write to file using atomic write for data integrity
        md_content = "\n".join(md)

        def _write_markdown(temp_file: Path) -> None:
            with open(temp_file, "w", encoding="utf-8") as f:
                f.write(md_content)

        write_result = atomic_file_write(output_path, _write_markdown)
        ...
        if write_result.success:
            ...
            logger.error(f"Failed to write Markdown report: {write_result.error}")
            raise ValueError(f"Cannot write report: {write_result.error}")
    ...
    def generate_sarif_report(result: AuditResult, output_path: str) -> dict[str, Any]:
        ...
        # Write to file using atomic write for data integrity
        def _write_sarif(temp_file: Path) -> None:
            with open(temp_file, "w", encoding="utf-8") as f:
                json.dump(sarif, f, indent=2)

        write_result = atomic_file_write(output_path, _write_sarif)

        if write_result.success:
            logger.info(
                f"SARIF report written to: {output_path} "
                f"({write_result.file_size} bytes, {write_result.duration_seconds:.2f}s)"
            )
        else:
            logger.error(f"Failed to write SARIF report: {write_result.error}")
            raise ValueError(f"Cannot write report: {write_result.error}")

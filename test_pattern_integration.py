...
class TestPatternDetectionIntegration:
    ...
    def test_patterns_detected_during_type_analysis(
        self, detector, sample_singleton_code
    ):
...
"""
        ...
        type_info = CSharpTypeInfo(
            name="Singleton", namespace="Core", file_path="test.cs", type_kind="class"
        )
        ...
        if result.design_patterns:
            singleton_patterns = [
                p for p in result.design_patterns if p["pattern"] == "singleton"
            ]
...
class TestPatternReportGeneration:
    ...
    def test_patterns_in_markdown_report(self, tmp_path, sample_csharp_project):
        ...
        report_text = CSharpAuditReporter.generate_markdown_report(
            result["audit_result"], result["types"], str(md_path)
        )
    ...
    def test_patterns_in_json_report(self, tmp_path, sample_csharp_project):
        ...
        report_dict = CSharpAuditReporter.generate_json_report(
            result["audit_result"], str(json_path), result["types"]
        )
        ...
        types_with_patterns = {
            k: v for k, v in result["types"].items() if v.design_patterns
        }
        if types_with_patterns:
...
"""
        ...
        type_info = CSharpTypeInfo(
            name="Repository", namespace="Data", file_path="test.cs", type_kind="class"
        )
        ...
        report_text = CSharpAuditReporter.generate_markdown_report(
            audit_result, {"Repository": result}, str(md_path)
        )

        # If patterns were detected, confidence should be in report
        if result.design_patterns:
            for _pattern in result.design_patterns:
...
"""
        ...
        type_info = CSharpTypeInfo(
            name="BadClass", namespace="Bad", file_path="test.cs", type_kind="class"
        )

...
- README.md"""
    ...
    def generate_structure(
        ...
        try:
            ...
            logger.warning(
                f"Error generating structure with LLM: {e}, using basic structure"
            )
            return self._generate_basic_structure(
                project_name, project_type, tech_stack
            )
    ...
    def _format_patterns(self, patterns: list[dict]) -> str:
        ...
        for i, pattern in enumerate(patterns, 1):
            ...
            document = (
                pattern.document if hasattr(pattern, "document") else str(pattern)
            )
            ...
            formatted.append(
                f"### Pattern {i}: {title} ({language})\n```\n{document[:PATTERN_PREVIEW_LENGTH]}...\n```\n"
            )

        return "\n".join(formatted)

    def _parse_structure(
        self, project_name: str, response_text: str
    ) -> ProjectStructure | None:
    ...
    def _generate_basic_structure(
        self, project_name: str, project_type: str, tech_stack: list[str]
    ) -> ProjectStructure:
    ...
    def _python_structure(
        self, name: str, ptype: str, stack: list[str]
    ) -> ProjectStructure:
...
""",
    ...
    def _java_structure(
        ...
        dirs = ["src/main/java", "src/main/resources", "src/test/java", package_path]
        files = {
            "README.md": f"# {name}\n\n{ptype.title()} built with {', '.join(stack)}\n",
